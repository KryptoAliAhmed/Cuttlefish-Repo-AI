 ...
 */
import type { Agent, Experiment } from "./Agent.js"
  ...
  evaluateExperiment(agent: Agent, experiment: Experiment, sensorData: any) {
    try {
      ...
      if (actual.ecological < expected.ecological * 0.8) {
        scoreAdjustment -= 12
        agent.log(
          `Experiment ${experiment.id} failed ecological metric (${actual.ecological} < ${expected.ecological * 0.8})`,
        )
      }
      ...
      const isValid = this.verifyZKP(
        agent.attestations.filter((a) => a.experimentId === experiment.id),
        sensorData,
      )
      if (!isValid) {
        scoreAdjustment -= 10
