...
export class Agent {
  ...
  proposeExperiment(description: string, projectedMetrics: Metrics, isHighRisk = false): Experiment {
  ...
  submitAttestation(experimentId: number, actualMetrics: Metrics) {
    ...
    this.log(
      `Submitted attestation for experiment ${experimentId}. Reputation: ${this.reputation} (Change: ${reputationChange})`,
    )
    return attestation
  }
  ...
  verifyExperiment(experimentId: number, sensorData: any): boolean {
    const experiment = this.experiments.find((e) => e.id === experimentId)
    if (!experiment) {
      this.log(`ERROR: Experiment ${experimentId} not found`)
      throw new ExperimentNotFoundError(`Experiment ${experimentId} not found`)
  ...
  private handleGrantTask(task: string, input: any) {
    ...
    const proposal = `Grant proposal for ${project} aligned with GCF requirements. Metrics: ${JSON.stringify(
      metricsAlignment,
    )}`
