...
import { LLMClient, LLMModel } from './LLMClient'; // Hypothetical LLM client with model selection
import { DAOContext } from './DAOContext'; // Hypothetical DAO context loader
import { TrustGraph } from './TrustGraph'; // Hypothetical vector DB for RAG
...
interface CWALayerConfig {
  ...
  model: LLMModel; // Assigned AI model
  validateContent: (content: string) => boolean; // Layer-specific validation
}
...
class BuilderAgentFactoryCWA {
  ...
  private defaultModel: LLMModel = 'gpt'; // Fallback model
  ...
  constructor(
    ...
    // Define CWA layers with AI assignments and validation
    this.cwaLayers = [
      {
        name: 'Instructions',
      ...
      },
      {
        ...
        alwaysInclude: true,
        model: 'gpt',
        validateContent: (content: string) => true, // Basic validation
      },
      {
      ...
      },
      {
        ...
        generateContent: async (data, llmClient, model) => {
          const relevantHistory = data.previousPlans.slice(-3);
          const prompt = `Summarize history: ${relevantHistory.map((p, i) => `Plan ${i + 1}: ${p}`).join('\n') || 'No prior history'}.`;
          return llmClient.generate(prompt, { maxTokens: 200, model });
        },
        priority: 6,
        alwaysInclude: false,
  ...
  private async buildContextWindow(userQuery: string): Promise<string> {
    ...
    for (const layerConfig of this.cwaLayers) {
      ...
      try {
        ...
        if (!层Config.validateContent(layerContent)) {
      ...
      if (currentTokens + layerTokens <= this.maxTokens || layerConfig.alwaysInclude) {
        ...
        logger.info({ correlationId: this.correlationId, layer: layerConfig.name, tokens: layerTokens }, 'Layer added to context');
      } else {
        logger.warn({ correlationId: this.correlationId, layer: layerConfig.name }, 'Skipping layer due to token limit');
      }
    }

    // Ensure User Query is last (recency effect)
