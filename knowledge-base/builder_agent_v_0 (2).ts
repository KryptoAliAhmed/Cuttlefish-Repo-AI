...
const DEFAULT_GAS_BUFFER_PERCENT = parseInt(process.env.DEFAULT_GAS_BUFFER_PERCENT || "20", 10);
...
async function simulateAndSubmitProposal(dao: string, calldata: string, threshold: number = 600) {
  ...
  if (!ethers.utils.isHexString(calldata) || calldata.length < 3) {
...
(async () => {
  if (mode === "submit") {
    ...
    if (!dao || !calldata) {
      ...
      console.log("Usage: ts-node builder_agent.ts submit <DAO_Address> <Calldata_Hex> [Reputation_Threshold_Optional]");
      process.exit(1);
    }
    ...
    if (!token || !target) {
      ...
      console.log("Usage: ts-node builder_agent.ts invest <Token_Address> <Target_Contract_Address> [Protocol_Name] [APY_BP] [Months] [Amount]");
      process.exit(1);
    }
    await forecastAndInvest(token, target, protocol, apyBP, months, amount);
  } else {
    console.error(`⛔ Unknown mode: "${mode}"`);
    console.log("Please use 'submit' or 'invest'.");
    process.exit(1);
  }
})();
