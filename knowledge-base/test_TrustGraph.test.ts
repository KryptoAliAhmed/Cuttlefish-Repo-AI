import { TrustGraph } from '../orchestrator/TrustGraph';
import { Agent, Metrics, Experiment } from '../orchestrator/Agent';
import { DAOSettings } from '../orchestrator/DAO';
import { ethers } from 'ethers';
import BuilderAgentWalletABI from '../artifacts/BuilderAgentWallet.json';
import { simpleHash } from '../orchestrator/utils';

jest.mock('ethers');
jest.mock('../orchestrator/utils', () => ({
...
describe('TrustGraph', () => {
  ...
  beforeEach(() => {
    ...
    agent = new Agent('agent1', { financial: 1000, ecological: 500, social: 300 });
    agent.daoId = 'AIClusterDAO-Azores';
  ...
  it('should add agent with initial trust score', () => {
    expect(trustGraph.getTrustScore('agent1')).toBe(100);
  });

  it('should evaluate experiment with positive adjustment using DAO ethical kernels', async () => {
    ...
    const expectedAdjustment = (10 * 0.55) + (8 * 0.30) + (6 * 0.15) + (5 * 0.55) + (4 * 0.30) + (3 * 0.15); // ethicalKernels + Genesys bonuses
  ...
  it('should apply penalty for poor performance and lock high-risk escrow', async () => {
    const experiment: Experiment = {
      id: 2,
  ...
  it('should handle contract call failures gracefully', async () => {
    ...
    expect(trustGraph.getTrustScore('agent1')).toBeGreaterThan(100); // Local trust still updates
  });
